...
class SageGCN(tf.keras.Model):
    ...
    def call(self, inputs):
        src_node_features, neighbor_node_features = inputs
    ...
    def call(self, node_features_list):
        ...
        for l in range(self.num_layers):
            ...
            for hop in range(self.num_layers - l):
                ...
                src_node_num = tf.shape(src_node_features)[0]

                #print("hop = ", hop, ", src_node_num = ", src_node_num)
                neighbor_node_features = tf.reshape(hidden[hop + 1], (src_node_num, self.num_neighbors_list[hop], -1))
                #print("src_node_features: ", tf.shape(src_node_features), ", neighbor_node_features: ", tf.shape(neighbor_node_features))
                h = gcn([src_node_features, neighbor_node_features])
                #print("gcn h: ", tf.shape(h))
